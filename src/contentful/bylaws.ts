    ...
    ? documentToHtmlString(richTextDocument, {
        ...
        renderNode: {
          ...
          [BLOCKS.TABLE]: (node, next) =>
            `<table class="min-w-full border-collapse border border-gray-300 my-4">${next(node.content)}</table>`,
          ...
          [BLOCKS.TABLE_HEADER_CELL]: (node, next) =>
            `<th class="border border-gray-300 px-4 py-2 bg-gray-100 font-bold">${next(node.content)}</th>`,
