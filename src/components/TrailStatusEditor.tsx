...
export default function TrailStatusEditor() {
  ...
  useEffect(() => {
    if (trails.length > 0) {
      setEditingTrails(trails.reduce((acc, trail) => ({...acc, [trail.id]: {...trail}}), {}));
    }
  }, [trails]);
      ...
      {trails.map((trail) => (
        <Card key={trail.id} sx={{mb: 2}}>
          <CardContent>
            ...
            >
              <Typography variant="h6">{trail.trail}</Typography>
              <FormControlLabel
                control={
                  <Switch
            ...
            <Box sx={{mt: 2, display: 'flex', justifyContent: 'flex-end'}}>
              <Button
                ...
                disabled={
                  updateTrailMutation.isPending && updateTrailMutation.variables?.id === trail.id
                }
                startIcon={
                  updateTrailMutation.isPending &&
                  updateTrailMutation.variables?.id === trail.id ? (
