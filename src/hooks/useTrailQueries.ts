...
export function useTrails() {
  ...
  const queryResult = useQuery({
    ...
    enabled: !cachedData || Date.now() - (queryClient.getQueryState(TRAILS_QUERY_KEY)?.dataUpdatedAt || 0) > 5 * 60 * 1000,
  });
...
export function useUpdateTrail() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      if (previousTrails) {
        queryClient.setQueryData<TrailData[]>(TRAILS_QUERY_KEY, 
          previousTrails.map(trail => 
