...
async function debugFirestoreMembers() {
  ...
  for (const userDoc of usersSnapshot.docs) {
    ...
    if (membershipsSnapshot.empty) {
      ...
      membershipsSnapshot.docs.forEach((membershipDoc, index) => {
        ...
        console.log(`         ├─ Stripe Subscription: ${membership.stripeSubscriptionId || 'N/A'}`);
        console.log(`         └─ Stripe Price: ${membership.stripePriceId || 'N/A'}\n`);
      });
    }
    ...
    if (cardDoc.exists) {
      ...
      if (card) {
        ...
        console.log(`   └─ ⚠️  Card document exists but has no data\n`);
      }
    } else {
  ...
  if (!statsDoc.exists) {
    ...
    if (stats) {
      ...
      console.log('   ⚠️  Stats document exists but has no data');
    }
  }
  ...
  if (totalMemberships === 0) {
    ...
    console.log("   Solution: Click 'Refresh Stats' button in Membership Management tab");
    console.log('   Or run: POST /api/admin/stats');
  } else if (statsDoc.exists) {
    const statsData = statsDoc.data();
    if (statsData && statsData.totalMembers !== totalMemberships) {
      ...
      console.log('\n⚠️  ISSUE: Stats document exists but has no data!');
    }
  }
  ...
  if (
    statsDoc.exists &&
    statsDoc.data() &&
    statsDoc.data()?.totalMembers === totalMemberships &&
    totalMemberships > 0
  ) {
